"""
Trains the chest triage model: is this chest X-ray normal or not.

Why this exists next to the multi label findings model:

The findings model answers "which of these eight things is present", and
the application was reading a normal or abnormal decision out of it by
checking whether any finding crossed its threshold. That decision was
poor. On a normal chest it still returned 84% for Atelectasis, so the
thresholds had to be pushed to 90 to keep it quiet, and anything close
to a threshold was reported as uncertain rather than normal.

This model answers the normal or abnormal question directly, which is
the question the triage screen actually asks.

Two things the earlier binary run got wrong are fixed here:

  1. It scored the test set at a fixed 0.5. The training set holds
     2.9 abnormal images for every normal one, so the model leans
     abnormal and 0.5 is the wrong place to cut. Recall on NORMAL came
     out at 0.688: nearly a third of normal chests were called abnormal.
     The cut point is now chosen on the validation set.

  2. It trained the head only. A second pass unfreezes the top of the
     backbone, which is where the chest specific detail is learned.

Run:

    python scripts/train_chest_binary_v2.py
    python scripts/train_chest_binary_v2.py --epochs 10 --fine-tune-epochs 6
"""
...
"""
DATA_DIR = AI_SERVICE_DIR / "data" / "chest_chexpert" / "processed"
TRAIN_DIR = DATA_DIR / "train"
VAL_DIR = DATA_DIR / "val"
TEST_DIR = DATA_DIR / "test"

OUTPUT_DIR = AI_SERVICE_DIR / "models" / "chest_triage_v4"
...
def main() -> None:
    ...
    """
    model_path = OUTPUT_DIR / "chest_triage_v4_model.keras"
    ...
    (OUTPUT_DIR / "chest_triage_v4_thresholds.json").write_text(
        json.dumps(
            {
                "threshold": threshold,
